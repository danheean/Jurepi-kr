...
콘텐츠는 **코드가 아니라 마크다운**이므로, 새 리스트를 넣거나 기존 리스트를 고치는 데 코드 변경이 필요 없습니다. 

**큐레이터는 리스트 단위로 지정**됩니다. 각 마크다운 파일의 frontmatter에 반드시 `curator` 필드를 넣어야 하며, 세 명 중 한 명을 선택합니다:
- `nuclear` — 갈곶동 핵주먹 (아바타: `public/images/restaurant-map/curators/nuclear.png`)
- `dragon` — 철산동 용주먹 (아바타: `public/images/restaurant-map/curators/dragon.png`)
- `honey` — 복현동 꿀주먹 (아바타: `public/images/restaurant-map/curators/honey.png`)

아바타는 256×256 PNG 이미지이며, UI에서는 지도 마커, 리스트 카드, 상세 패널 곳곳에 표시됩니다. KO 파일에만 `curator`를 쓰면 EN 파일은 자동으로 상속됩니다.
