...
<platform_integration>
  - 라우트: /[locale]/tools/cron-parser (SSG; 레지스트리 슬러그 "cron-parser", id "cron-parser", 상태 "coming_soon", 악센트 "rose", 카테고리 "dev").
...
1. **레지스트리 항목**: id/slug `cron-parser`, 카테고리 `dev`, 악센트 `rose`, 상태 `coming_soon`, `addedAt: '2026-07-10'`(필수). 순서 = 22(필요 시 조정).
