...
<aesthetic_guidelines>
  ...
  <direction>밝고, 친근하며, 경쾌하고 활력 있는 라이트 테마를 메인으로 지향합니다. 깨끗한 흰색 바탕에 은은하게 퍼지는 브랜드 톤의 소프트 그림자(Shadow)를 가미한 카드 레이아웃을 사용합니다. 6개의 카테고리별 개성 넘치는 포인트 컬러를 각 카드의 아이덴티티 강조색으로 부여하며, 기본 버튼 및 링크 등의 클릭 요소는 브랜드 바이올렛(#6C5CE7) 컬러로 고정 통일합니다.</direction>
  <main_screen_usage>
    - 히어로 타이틀(H1) 영역은 Gmarket Sans 폰트를 활용하여 개성 있고 뚜렷하게 연출하며, 본문과 인터페이스 요소에는 Pretendard 글꼴을 공통 적용합니다.
    - 도구 카드의 아이콘 타일 배경과 배지 영역은 해당 도구가 속한 카테고리의 지정 강조색(coral/mint/sky/sun/grape/rose) 소프트 컬러를 영리하게 녹여내어 개성을 살리고, CTA 및 실행 버튼은 통일된 브랜드 바이올렛 계열을 일관되게 사용합니다.
