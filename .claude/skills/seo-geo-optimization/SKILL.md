...
4. **`llms.txt`.** 사이트 루트 `/llms.txt`에 도구 목록·설명·URL을 마크다운으로 큐레이션해 LLM에게 사이트 지도를 제공(신흥 표준). `public/llms.txt`로 정적 서빙.
   **`llms.txt`는 정적 파일이라 빌드가 검사하지 않는다 — 도구가 늘어도 자동으로 갱신되지 않고 조용히 드리프트한다.** cheer 세션 실측: 레지스트리 `status:'live'` 도구가 26개인데 `llms.txt`엔 23개만 등재돼 3개(`bookmarks`·`restaurant-map`·`unit-converter`)가 누락돼 있었다 — 그 도구들이 출시될 때 `llms.txt` 갱신을 빠뜨린 것, tsc·유닛·빌드·프리렌더 전부 무관(정적 텍스트 파일이라 아무 게이트도 안 본다). **예방:** 신규 도구 출시 체크리스트(아래)에 `llms.txt` 항목이 있지만, 그것만으론 부족 — 주기 점검 시 `grep -c "https://apps.jurepi.kr/ko/tools/" public/llms.txt` 결과가 `grep -c "status: 'live'" src/tools/registry.ts`와 일치하는지 대조하고, 신규 항목은 `en.json`의 `tools.<slug>.description`을 근거로 작성한다(마케팅 문구 재작성 금지, 실제 기능 설명 그대로).
