...
|------|------|------|--------|----------------|
| 1 설계 | architect | PRD/DESIGN, 기존 `_workspace/` | `NN_architect_{feature}-blueprint.md`(계층표·계약·불변식·작업분배) | 예 — 계약이 모든 구현의 전제 |
...
## 3. 권장 빌드 순서 (초기 구축, PRD 반영)

PRD의 `recommended_implementation_order`를 계층 파이프라인에 맞춘 것:
...
1. architect: 새 도구의 계층 분해(도메인 로직 유무 판단) + 자체 PRD 확인.
