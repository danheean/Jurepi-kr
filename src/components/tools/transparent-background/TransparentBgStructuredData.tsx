...
/**
 * Transparent Background Maker structured data (JSON-LD) component.
 * Injects SoftwareApplication JSON-LD for search engines and AI crawlers.
 * FAQPage JSON-LD is emitted by TransparentBgFaq (single owner, matches every other tool).
 * Rendered at the route level (not wrapped in mounted gate) for full discoverability.
 */
export function TransparentBgStructuredData() {
  const locale = useLocale() as 'ko' | 'en';
  const t = useTranslations('tools.transparent-background');

  const toolUrl = absoluteToolUrl(locale, 'transparent-background');
