...
import {
  ...
  type SpeedQuizStore,
  type MergedDeck,
} from '@/lib/speed-quiz/schema';
import {
  createCatalog,
  CATEGORY_ORDER,
} from '@/lib/speed-quiz/catalog';
import {
  toggleFavorite,
  pushRecent,
  pruneUnknown,
  RECENTS_MAX,
} from '@/lib/speed-quiz/favorites';
import { startGame, markCorrect, markPass, undo, endGame, type GameState, type WordResult } from '@/lib/speed-quiz/game-reducer';
import { toneSpec } from '@/lib/speed-quiz/sound';
import { useReducedMotion } from '@/hooks/useReducedMotion';

const STORAGE_KEY = 'jurepi-speed-quiz';
const SEARCH_DEBOUNCE = 120;

/**
 * Simple search filter for decks by title/category
 */
...
export interface UseSpeedQuizReturn {
  ...
  // Setup settings (persisted subset)
  settings: {
  ...
  setSetting: <K extends keyof UseSpeedQuizReturn['settings']>(
    key: K,
    value: UseSpeedQuizReturn['settings'][K]
  ) => void;
  startGame: () => void;

  // Active game (localized words already resolved by locale)
  game: GameState | null;
...
interface State {
  ...
  game: GameState | null;
  // Setup settings (mutable within setup phase, persisted on game start)
  setupSettings: {
    difficulty?: 'easy' | 'normal' | 'hard';
    roundTimeSeconds: number | null;
    shuffle: boolean;
...
type Action =
  ...
  | { type: 'SET_STORE'; payload: SpeedQuizStore }
  | { type: 'SET_MOUNTED' }
  ...
  | { type: 'TOGGLE_SOUND' }
  | { type: 'SYNC_STORE'; payload: SpeedQuizStore };
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'OPEN_SETUP': {
      ...
      // Pre-fill difficulty from deck if single difficulty
      const diffFromDeck = deck.difficulty;
      return {
        ...state,
        phase: 'setup',
    ...
    case 'MARK_CORRECT': {
      ...
      // Follow the reducer's status: last word transitions to summary.
...
export function useSpeedQuiz(): UseSpeedQuizReturn {
  ...
  useEffect(() => {
    const loadCatalog = async () => {
      try {
        const module = await import('./data/speed-quiz.generated.json');
        const decks = (module.default || module) as MergedDeck[];
        dispatch({ type: 'SET_CATALOG', payload: decks });

        // Load localStorage
        const stored = localStorage.getItem(STORAGE_KEY);
        let store: SpeedQuizStore;
        if (stored) {
          try {
            const parsed = JSON.parse(stored);
            const validated = SpeedQuizStoreSchema.safeParse(parsed);
  ...
  // Persist store to localStorage (eager, non-debounced)
  const persistStore = useCallback((storeData: SpeedQuizStore) => {
    try {
  ...
  // Timer management (rAF countdown)
  useEffect(() => {
  ...
  // Current word with localization
  const currentWord =
    state.game && state.game.currentIndex < state.game.words.length
      ? (() => {
          ...
          return {
            ...
            hint:
              state.setupSettings.showHints ? (localeWord?.hint || w.hint) : undefined,
          };
        })()
      : null;
    ...
    state.game && state.game.status === 'summary'
      ? state.game.words.map((w, idx) => ({
          term:
            locale === 'ko'
              ? state.catalog
                  .find((d) => d.slug === state.game!.deckId)
                  ?.ko.words[idx]?.term || w.term
              : state.catalog
                  .find((d) => d.slug === state.game!.deckId)
                  ?.en.words[idx]?.term || w.term,
          result: w.result,
        }))
      : [];
  ...
  return {
    ...
    setSetting: (key, value) =>
      dispatch({ type: 'SET_SETUP_SETTING', payload: { key, value } }),
    startGame: () => {
      if (!state.selectedSlug) return;
      const deck = state.catalog.find((d) => d.slug === state.selectedSlug);
      if (!deck) return;
      const seed = Math.floor(Math.random() * 0xffffffff);
      const newGame = startGame(deck, {
    ...
    timerMs: timerMs,
    roundTimeMs: state.setupSettings.roundTimeSeconds
      ? state.setupSettings.roundTimeSeconds * 1000
      : null,
